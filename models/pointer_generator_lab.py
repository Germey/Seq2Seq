...
class PointerGeneratorLabModel():
            ...
            if self.mode == 'train':
                ...
                with tf.variable_scope('loop', reuse=tf.AUTO_REUSE):
                    ...
                    for _ in range(self.decoder_max_time_steps):
                        ...
                        # if len(self.decoder_predicts) == 0 or :
                        #     self.decoder_predicts.append(predict)、
                        #     self.logger.debug('Decoder Predicts %s', 、self.decoder_predicts)
                        
                        # if tf.reduce_sum(tf.cast(tf.equal(tf.cast(predict, tf.float32),
                        #                                   tf.stack(self.decoder_history)), tf.float32)).eval(
                        #     sess=self.temp_sess) > 0:
                        #     print('666666')
                        # if predict > 0:
                        #     print('66666666')
                        
                        predicts_top_k = tf.nn.top_k(final_distribution, 2).indices
                        # print('')
                        self.logger.debug('predicts_top_k %s', predicts_top_k)
                        # if not last_predict and:
