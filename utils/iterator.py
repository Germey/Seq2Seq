...
class UniTextIterator():
    ...
    def extend(self, source):
        ...
        return source_ids_extend, oovs_vocab
    ...
    def next(self, extend=False):
        ...
        while not self.end_of_data:
            ...
            if source_item:
                ...
                if extend:
                    source_ids_extend, oovs_vocab = self.extend(source_item)
                    if len(oovs_vocab) > oovs_max_size:
                        oovs_max_size = len(oovs_vocab)
                
                if self.max_length and len(source_ids) > self.max_length:
                    continue
                if self.skip_empty and not source_ids:
                    continue
            ...
            # end or batch exceed
            if self.end_of_data and len(source) or len(source) >= self.batch_size:
                if not extend:
                    yield source
                    source = []
                else:
                    yield source, source_extend, oovs_max_size, oovs_vocabs
                    source, source_extend, oovs_vocabs = [], [], []
                    oovs_max_size = 0
...
class BiTextIterator():
    ...
    def next(self, extend=False):
        ...
        source, target = [], []
        if extend:
            source_extend, target_extend, oovs_vocabs = [], [], []
            oovs_max_size = 0
        ...
        while not self.end_of_data:
            ...
            if self.end_of_data and len(source) and len(target) or \
                len(source) >= self.batch_size and len(target) >= self.batch_size:
                if extend:
